...
import { Star, MapPin, Eye, Heart, CheckCircle, Calendar } from "lucide-react";
import { RentalItem } from "@/types/category";

interface RentalResultCardProps {
  item: RentalItem;
  viewMode: 'grid' | 'list';
}

export function RentalResultCard({ item, viewMode }: RentalResultCardProps) {
  return (
          ...
          <div className="flex-1">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <Badge className="bg-accent text-accent-foreground">
                    ...
                    <span className="mr-1">إيجار</span>
                  </Badge>
            ...
            <div className="flex items-center justify-between">
              <div className="text-left">
                ...
                {item.available && <Badge className="bg-success text-success-foreground text-xs">متاح</Badge>}
              </div>
              <div className="flex gap-2">
                <Button variant="outline" size="sm" className="glass-hover border-primary/30"><Heart className="h-3 w-3" /></Button>
                <Link to={`/rental/${item.id}`}><Button size="sm" className="bg-primary hover:bg-primary/90 text-arabic"><Eye className="ml-2 h-3 w-3" />{viewMode === 'list' ? 'عرض التفاصيل' : 'عرض'}</Button></Link>
              </div>
            </div>
          </div>
        </div>
